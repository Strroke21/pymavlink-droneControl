...
def formation_row(): #rectangle formation
    while True:
        ######## leader ######
        ...
        if data[0] == 3:
            ...
            if current_pos:
                ...
                print(lead_yaw)
                ######### follower3 ######
                yaw1 = 135  #on the right
                dist1 = 28.28 #change this distance to add new follower
                lat1, lon1 = relative_pos(lat_lead, lon_lead, dist1, lead_yaw, yaw1)
                print(lat1, lon1)
                ##### send msg to follower3 drone #############

                goto_waypoint(follower3,lat1,lon1,10) #altitude 1
                print("formation command sent.")
                time.sleep(0.15)
                dist_to_target = distance_between(follower3,lat1,lon1)
...
follower3 = connect('/dev/ttyACM0')
print("follower3 connected.")

# follower3
VehicleMode(follower3,"GUIDED")
print("follower3 in GUIDED mode")
time.sleep(1)

#arm
arm(follower3)
print("arming the follower3")
time.sleep(5)

#takeoff
drone_takeoff(follower3,10)
print("taking off follower3")

while True:
    altitude = abs(get_local_position(follower3)[2])
    print(f"Altitude: {altitude} m.")
    if altitude>9:
        print("Target altitude reached.")
        break

relativePosition = 'bottom_right' #follower3 in the straight right of the leader
...
while True:    
    ...
    if pos[0]==3:
        ...
        yaw1 = 135 #yaw 135 for backward right relative point calculation

        follower_offset_y = 28.28 #offset in right #change this distance to add new follower
        follower_offset_x = 5
        send_velocity_setpoint(follower3,lead_vx,lead_vy,0) #sending only vx and vy
        print(f"leader vx: {lead_vx} m/s leader vy: {lead_vy} m/s leader vz: {lead_vz} m/s")
        time.sleep(0.15)
        lat1, lon1 = relative_pos(leader_lat,leader_lon, follower_offset_y, leader_yaw, yaw1) #coordinates in 20m backward direction
        #print(f"Leader Coordinates: Latitude: {leader_lat} Longitude: {leader_lon}")
        distance_v = abs(distance_between(follower3,lat1,lon1)) #vertical distance to leader
        distance_h = abs(( (distance_between(follower3,leader_lat,leader_lon))**2 - (distance_v)**2 )**(1/2))  #horizontal distance to leader
        diff_dist_y = follower_offset_y - distance_h  
        diff_dist_x = distance_v - follower_offset_x
        print(f"x-difference: {round(diff_dist_x,2)} m. y-difference: {round(diff_dist_y,2)} m.")

        if abs(diff_dist_y)>=1: #formation correction if difference is greater than 1m. 
            send_velocity_setpoint(follower3,0,diff_dist_y,0) #formation velocity-y command
        ...
        if abs(diff_dist_x)>=1: #formation correction if difference is greater than 1m. 
            pos_f = get_global_position(follower3)
            lat_f = pos_f[0]
            lon_f = pos_f[1]
            ang = angle_difference_with_heading(leader_lat,leader_lon,leader_yaw,lat_f,lon_f)

            if (135<ang>90): #if angle is between 135 and 90deg  then drone will go backward
                send_velocity_setpoint(follower3,-diff_dist_x,0,0)
                print("velocity vx command sent for formation.")
                time.sleep(0.2)

            if (180<ang>135): #if angle is between 180 and 135deg then drone will go forward
                send_velocity_setpoint(follower3,diff_dist_x,0,0)
                print("velocity vx command sent for formation.")
                time.sleep(0.2)
